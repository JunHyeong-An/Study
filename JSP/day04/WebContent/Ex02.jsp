...
<%
	...
	// pageContext, reqeust, session, application은 Object를 저장해두었다가 꺼내 쓸 수 있는 내장객체
	// 범위(scope)는 순서대로 pageContext가 제일 작고, application이 가장 크다.
	...
	// forward 형식으로 페이지를 변경한 경우에서는 request가 유지된다.
	
	session.setAttribute("address", "부산광역시 해운대구");
	// 세션을 강제로 종료시키거나(로그아웃), 클라이언트가 브라우저를 완전 종료하기 전까지 유효하다.
	// 서로 다른 페이지에서도 같은 객체에 접근 가능하기 때문에, 활용도가 높다.
	...
	// getArrtibute(String name)을 사용해서 값을 불러 올 수 있다. - Object 반환, 자바의 모든 객체를 처리할 수 있다.(down-casting)
	// getParameter(String name) - String을 반환, 문자열 데이터를 처리한다.
